...
import {
  ...
  type ConnectServiceConfig,
} from './ConnectServiceFactory';
...
describe('ConnectServiceFactory', () => {
  ...
  describe('validateConfig', () => {
    it('should validate valid configuration', () => {
      expect(() =>
        ConnectServiceFactory.validateConfig(validConfig)
      ).not.toThrow();
    });
    ...
    it('should throw error for missing region', () => {
      ...
      expect(() => ConnectServiceFactory.validateConfig(config)).toThrow(
    ...
    it('should throw error for missing instanceId', () => {
      ...
      expect(() => ConnectServiceFactory.validateConfig(config)).toThrow(
        'AWS Connect instance ID is required'
      );
    });
    ...
    it('should throw error for missing contactFlowId', () => {
      ...
      expect(() => ConnectServiceFactory.validateConfig(config)).toThrow(
        'AWS Connect contact flow ID is required'
      );
    });
    ...
    it('should throw error for invalid instanceId format', () => {
      ...
      expect(() => ConnectServiceFactory.validateConfig(config)).toThrow(
        'Invalid AWS Connect instance ID format'
      );
    });
    ...
    it('should throw error for invalid contactFlowId format', () => {
      ...
      expect(() => ConnectServiceFactory.validateConfig(config)).toThrow(
        'Invalid AWS Connect contact flow ID format'
      );
    });
    ...
    it('should accept valid region formats', () => {
      ...
      validRegions.forEach(region => {
        const config = { ...validConfig, region };
        expect(() =>
          ConnectServiceFactory.validateConfig(config)
        ).not.toThrow();
      });
    });
