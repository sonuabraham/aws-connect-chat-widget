...
const createExtendedTheme = (theme: ThemeConfiguration): ExtendedTheme => {
  ...
  return {
    ...
    shadowMedium: '0 3px 6px rgba(0, 0, 0, 0.15), 0 2px 4px rgba(0, 0, 0, 0.12)',
    shadowLarge: '0 10px 20px rgba(0, 0, 0, 0.15), 0 3px 6px rgba(0, 0, 0, 0.10)',
 ...
 */
const generateCSSCustomProperties = (theme: ExtendedTheme): Record<string, string> => {
  return {
    // Colors
...
export const useResponsive = () => {
  ...
  return {
    ...
    // Utility functions
    isMobile: () => window.matchMedia(`(max-width: ${theme.breakpoints.mobile})`).matches,
    isTablet: () => window.matchMedia(`(max-width: ${theme.breakpoints.tablet})`).matches,
    isDesktop: () => window.matchMedia(`(min-width: ${theme.breakpoints.desktop})`).matches
  };
};
 ...
 */
export const createStyles = (stylesFn: (theme: ExtendedTheme) => Record<string, React.CSSProperties>) => {
