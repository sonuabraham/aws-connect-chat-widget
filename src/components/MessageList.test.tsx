...
describe('MessageList', () => {
  ...
  describe('Message Bubbles', () => {
    it('should render visitor messages with correct styling', () => {
      const messages = [
        createMessage({ content: 'Visitor message', sender: 'visitor' }),
      ];

      render(<MessageList {...defaultProps} messages={messages} />);

      const messageBubble = screen
        .getByText('Visitor message')
    ...
    it('should render agent messages with correct styling', () => {
      const messages = [
        createMessage({ content: 'Agent message', sender: 'agent' }),
      ];

      render(<MessageList {...defaultProps} messages={messages} />);

      const messageBubble = screen
        .getByText('Agent message')
        .closest('.message-bubble');
    ...
    it('should not show agent name/avatar for visitor messages', () => {
      ...
      expect(
        screen.queryByAltText('John Smith avatar')
  ...
  describe('Message Status Icons', () => {
    it('should show sending status icon', () => {
      const messages = [
        createMessage({ status: 'sending', sender: 'visitor' }),
      ];
    ...
    it('should show delivered status icon', () => {
      const messages = [
        createMessage({ status: 'delivered', sender: 'visitor' }),
      ];
    ...
    it('should show failed status icon with error styling', () => {
      ...
      const statusIcon = document.querySelector(
        '.message-bubble__status-icon--error'
      );
      expect(statusIcon).toBeInTheDocument();
    });
    ...
    it('should not show status icons for agent messages', () => {
      const messages = [
        createMessage({ status: 'delivered', sender: 'agent' }),
      ];
  ...
  describe('Typing Indicator', () => {
    it('should show typing indicator when isTyping is true', () => {
      ...
      expect(document.querySelectorAll('.typing-indicator__dot')).toHaveLength(
        3
      );
    });
    ...
    it('should not show typing indicator when isTyping is false', () => {
      ...
      expect(
        screen.queryByText('John Smith is typing...')
      ).not.toBeInTheDocument();
    });

    it('should show generic typing message when no agent name', () => {
      render(
        <MessageList {...defaultProps} isTyping={true} agentInfo={undefined} />
      );
  ...
  describe('Empty State', () => {
    ...
    it('should hide empty state when messages exist', () => {
      ...
      expect(
        screen.queryByText('Start a conversation by sending a message below.')
      ).not.toBeInTheDocument();
    });
  });
