...
describe('AgentInfo', () => {
  ...
  describe('Agent Information Display', () => {
    it('should display agent name and profile image when connected', () => {
      ...
      expect(screen.getByAltText('John Smith profile')).toHaveAttribute('src', 'https://example.com/avatar.jpg');
    });
    ...
    it('should display fallback avatar when profile image is not provided', () => {
      ...
      expect(screen.getByText('John Smith')).toBeInTheDocument();
      expect(screen.queryByAltText('John Smith profile')).not.toBeInTheDocument();
      // Fallback avatar should be visible
      expect(document.querySelector('.agent-info__avatar--fallback')).toBeInTheDocument();
    });
    ...
    it('should display placeholder when no agent and not connected', () => {
      ...
      expect(document.querySelector('.agent-info__avatar--placeholder')).toBeInTheDocument();
    });

    it('should return null when no agent and connected', () => {
      const { container } = render(<AgentInfo agent={undefined} isConnected={true} />);
  ...
  describe('Typing Indicator', () => {
    it('should show typing indicator when agent is typing and showTypingIndicator is true', () => {
      ...
      render(<AgentInfo agent={typingAgent} isConnected={true} showTypingIndicator={true} />);
      
      expect(screen.getByText('typing...')).toBeInTheDocument();
      expect(screen.getByRole('status')).toBeInTheDocument();
      expect(document.querySelectorAll('.agent-info__typing-dot')).toHaveLength(3);
    });

    it('should not show typing indicator when showTypingIndicator is false', () => {
      const typingAgent = { ...mockAgent, isTyping: true };
      render(<AgentInfo agent={typingAgent} isConnected={true} showTypingIndicator={false} />);
  ...
  describe('Agent Disconnection Handling', () => {
    it('should call onAgentDisconnected when agent goes offline', async () => {
      ...
      const { rerender } = render(
        <AgentInfo {...defaultProps} onAgentDisconnected={onAgentDisconnected} />
      );
  ...
  describe('Compact Mode', () => {
    it('should apply compact styling when compact prop is true', () => {
      ...
      expect(document.querySelector('.agent-info--compact')).toBeInTheDocument();
    });

    it('should not apply compact styling when compact prop is false', () => {
      render(<AgentInfo {...defaultProps} compact={false} />);
      
      expect(document.querySelector('.agent-info--compact')).not.toBeInTheDocument();
    });
  });
  ...
  describe('Accessibility', () => {
    ...
    it('should truncate long agent names with title attribute', () => {
      const longNameAgent = { ...mockAgent, name: 'This is a very long agent name that should be truncated' };
      render(<AgentInfo agent={longNameAgent} isConnected={true} />);
      
      const nameElement = screen.getByText('This is a very long agent name that should be truncated');
      expect(nameElement).toHaveAttribute('title', 'This is a very long agent name that should be truncated');
    });
  });
